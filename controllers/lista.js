...
function newList(req, res) {
    ...
    list.save((err, listCreated) => {
        if (err) {
            res
                .status(500)
                .send({ accion: 'new list', error: 'Lista con esa contraseña ya existe' });
        } else {
...
function editList(req, res) {
    ...
    Lista.findOneAndUpdate({ pass: params.pass }, { $set: { lista: params.lista } }, { new: true },
        (err, renamedList) => {
            if (err) {
                res
                    .status(500)
...
function delItem(req, res) {
    ...
    Lista.update({ pass: params.pass }, { $pull: { items: { _id: itemId } } }).exec((err, delItem) => {
        if (err) {
            res
                .status(500)
